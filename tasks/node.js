...
module.exports = function(grunt) {
  ...
  grunt.registerTask('node', function() {
    ...
    var output = grunt.template.process(read(nodeConfig.wrapper), {
      content: read(grunt.template.process(nodeConfig.content))
    });


    // Write the contents out
    grunt.file.write("dist/node/miso.events." + grunt.template.process(grunt.config("pkg").version) + ".js", output);
  });
